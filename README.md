...
2. 검색 태그 준비 (`musinsa_snap_clooecter.json`)

   기본 제공되는 `musinsa_snap_clooecter.json`에는 남녀 14개 카테고리(아우터/상의/셔츠/니트/팬츠/데님/신발)가 포함되어 있습니다. 원하는 검색어를 추가하거나 교체할 수 있습니다. 예를 들어, 사용자 환경의 경로가 `C:\\Users\\김종원\\OneDrive - 티비에이치글로벌\\Python_data\\python_File\\무신사_SNAP\\musinsa_snap_clooecter.json`이라면 해당 파일을 그대로 지정하면 됩니다.
...
3. 자동화 실행 예시 (남/여 필터 포함)
   ...
   python -m musinsa_snap.automation \
     --tags-file musinsa_snap_clooecter.json \
     --gender M \
     --limit 10 \
   ...
   - 메인 검색창에 `musinsa_snap_clooecter.json`의 검색어를 순차로 입력 후 SNAP/코디 탭 선택 (Selenium이 검색/탭 클릭을 수행)
   - 필터에서 남/여 버튼을 클릭해 성별을 고정
   - 최좌상단 SNAP 이미지를 한 번 열어 정상 연결을 확인한 뒤 닫고, 스크롤하며 **노출 순서 그대로 상위 10개** 스냅 링크를 수집
   - 각 스냅 상세를 새 탭으로 열어 파란색 해시태그까지 모으고, 필요 시 렌더된 DOM을 한 번 더 확인해 빠진 이미지/태그를 보충한 뒤 이미지 다운로드/CSV(`query,id,url,title,author,taken_at,tags,image_path`)/JSON(`snaps.json`) 저장
